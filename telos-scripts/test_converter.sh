...
$COMMAND push action zarrelay.tbn transfer '["admin.tbn","'${NRTWORK}'","'${SMART_AMOUNT}' '${SMART_TOKEN}'","1,'${CONVERTER}' TLOS,0.00000010,admin.tbn"]' -p admin.tbn@active

echo
sleep 1
echo -e "${CYAN}---------------------------'${SMART_TOKEN}'=>${SYMBOL}---------------------------${NC}"
$COMMAND push action zarrelay.tbn transfer '["admin.tbn","'${NRTWORK}'","'${SMART_AMOUNT}' '${SMART_TOKEN}'","1,'${CONVERTER}' '${SYMBOL}',0.00000010,admin.tbn"]' -p admin.tbn@active

echo
sleep 1
echo -e "${CYAN}----------------------------TLOS=>${SYMBOL}-----------------------------${NC}"
$COMMAND push action eosio.token transfer '["admin.tbn","'${NRTWORK}'","'${TLOS_AMOUNT}' TLOS","1,'${CONVERTER}' '${SYMBOL}',0.01,admin.tbn"]' -p admin.tbn@active

echo
sleep 1
echo -e "${CYAN}----------------------------${SYMBOL}=>TLOS-----------------------------${NC}"
$COMMAND push action stablecoin.z transfer '["admin.tbn","'${NRTWORK}'","'${TOKEN_AMOUNT}' '${SYMBOL}'","1,'${CONVERTER}' TLOS,0.0001,admin.tbn"]' -p admin.tbn@active
